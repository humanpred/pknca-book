...
flowchart TD
    ...
    S2["② PKNCAconc()<br/>define conc column, time column,<br/>grouping variables, route-specific args"] --> S3
...
Below is the complete workflow with the built-in `Theoph` dataset (oral theophylline, 12 subjects). The package vignette [Computing NCA Parameters for Theophylline](https://humanpred.github.io/pknca/articles/v02-example-theophylline.html) works the same dataset.
...
`PKNCAdata` combines concentration and dose objects. If you don't specify `intervals`, PKNCA builds default intervals from the built-in `single.dose.aucs` specification, anchored at the dosing times.
...
| `tobit_n_points_penalty` | `0` | Penalty per extra terminal point in Tobit log-likelihood; positive values favor shorter windows (≥ 0.12.2) |
...
`as.data.frame()` returns one row per parameter per interval — a tidy shape ready for dplyr and ggplot2. More result-handling patterns: [Post-Processing](https://humanpred.github.io/pknca/articles/v07-post-processing.html).
