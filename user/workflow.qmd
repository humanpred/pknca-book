...
PKNCA typically takes two separate data frames: one for concentrations and one for doses (dose data are optional, but required for dose-dependent parameters such as CL and Vz; without dose data, you must supply `intervals` explicitly, because automatic intervals need dose times). Both must be in **long format** (one row per observation).
...
`concentration ~ time | group1 + group2 + subject`

Additional grouping levels chain with `+`, and the last grouping variable is treated as the subject. An optional `/analyte` after the subject declares a multi-analyte structure (`conc ~ time | Study + Subject / Analyte`), with the element before the `/` treated as the subject.
...
Override for a single analysis by passing `options` to `PKNCAdata()`. For rerunning an analysis, `PKNCAresults` objects have an `update()` method, but its fast path is narrow: when only the **concentration or dose data** change, `update(o_nca, data = new_data)` detects which groups' data changed and recalculates just those, keeping every other result. Any other change — different `intervals`, different `options`, or different column definitions — triggers a full recalculation with a warning. So to rerun with different settings, you still build a new `PKNCAdata` object with the new `options` and call `pk.nca()` again:
...
**pkgdown reference:** [PKNCAconc()](https://humanpred.github.io/pknca/reference/PKNCAconc.html) · [PKNCAdose()](https://humanpred.github.io/pknca/reference/PKNCAdose.html) · [PKNCAdata()](https://humanpred.github.io/pknca/reference/PKNCAdata.html) · [pk.nca()](https://humanpred.github.io/pknca/reference/pk.nca.html) · [as.data.frame()](https://humanpred.github.io/pknca/reference/as.data.frame.PKNCAresults.html) · [summary()](https://humanpred.github.io/pknca/reference/summary.PKNCAresults.html) · [update()](https://humanpred.github.io/pknca/reference/update.PKNCAresults.html) · [PKNCA.options()](https://humanpred.github.io/pknca/reference/PKNCA.options.html) · [exclude_nca_min.hl.r.squared()](https://humanpred.github.io/pknca/reference/exclude_nca.html)
:::
