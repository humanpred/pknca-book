...
| `aucall` | Through the first BLQ after Tlast (later BLQs contribute zero) | Post-last-measurable BLQs treated as 0 | When you want to include the BLQ tail |
...
In PKNCA ≥ 0.12.2 the AUMC family mirrors the AUC families: interval AUMC (`aumcint.last`, `aumcint.all`, `aumcint.inf.obs`, `aumcint.inf.pred`, plus their `.dose` variants with dose-aware boundary interpolation) and IV AUMC with C0 back-extrapolation (`aumciv*`, e.g. `aumcivlast`, `aumcivinf.obs`) are available and requested like any other interval parameter.
